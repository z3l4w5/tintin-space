...
我们可以使用页面的文件夹位置来查找其部分。 如果我们为新闻页面运行 {{path.Base (path.Dir .Page.FirstSection.File.Path)}}，我们将获得新闻部分。 .Page.FirstSection指向新闻部分的_index页面，我们可以使用路径.Dir在其路径中找到文件夹名。 请注意，我们也可以将CurrentSection用于新闻页面，但这将破坏博客部分的社区小节。 我们可以使用这个文件夹位置来更改News页面的HTML类，并使其不同于其他页面，如清单7.2所示。

{{< hint info >}}
**TIP** 最大限度地减少在前端输入数据的需求可以帮助保持内容区域的简单。 在创建内容时，我们必须考虑的前置问题元数据越少越好。 我们做了一次主题，但我们不断向网站添加内容。
{{< /hint >}}
