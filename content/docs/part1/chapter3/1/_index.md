...
3.3 shows these elements and how they are rendered in the browser. To create toplevel headings,
...
Markdown provides basic formatting support using text-based markers. We can surround the desired text with an _(underscore) or an *(asterisk) for italics, or a (double underscore) for boldface. For both bold and italics, we can use **_<text>_** or
...
Markdown creates &lt;img> tags to show images inline with the content. There is no support for specifying block images, image dimensions, or other details; that is left to the theme to implement. ![Figure3.5](Figure3.5.svg)

Figure 3.5 Elements in Markdown, part 3: Inline elements including bold, italics, underline, links, and images

We can inline images using a similar syntax to links and prefix those with an ! (exclamation mark, commonly called a bang). To render an image inline in the text, we’d write \!\[alt text](/path/to/image "optional tool- tip"). We can use relative paths in the image tag as well.
...
In the Privacy Policy page, we used TM in the first line to indicate Acme Corporation’s trademark. Now would be a good time to replace it with &trade; to get the HTML trademark symbol. Note that you can also insert direct Unicode characters like
